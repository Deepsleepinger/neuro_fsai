...
def algebraic_graph_features(A, row, col, values, eps=1e-12, clip=10.0):
    """Build permutation-equivariant algebraic features.

    These features avoid absolute row ids and index-distance features. Extra
    candidate edges that are not present in A should pass value 0.0.
    """
    A = canonicalize_sparse_matrix(A)
    abs_A = abs(A).tocsr()
    ...
    row_abs_sum = np.asarray(abs_A.sum(axis=1)).reshape(-1).astype(np.float64)
    col_abs_sum = np.asarray(abs_A.sum(axis=0)).reshape(-1).astype(np.float64)
    offdiag_abs_sum = np.maximum(row_abs_sum - abs_diag, 0.0)

    row_degree = np.diff(A.indptr).astype(np.float64)
    col_degree = np.diff(A.tocsc().indptr).astype(np.float64)
    diag_dom = abs_diag / (offdiag_abs_sum + eps)

    node_attr = np.stack([
        np.log1p(abs_diag),
        np.sign(diag),
        np.log1p(np.minimum(diag_dom, 1e6)),
        np.log1p(row_degree),
        np.log1p(col_degree),
    ], axis=1).astype(np.float32)

    scale = np.sqrt(abs_diag[row] * abs_diag[col] + eps)
    rel_value = values.astype(np.float64, copy=False) / scale
    if clip is not None and clip > 0:
        rel_value = np.clip(rel_value, -clip, clip)

    row_rel_abs = np.abs(values) / (row_abs_sum[row] + eps)
    col_rel_abs = np.abs(values) / (col_abs_sum[col] + eps)
    edge_attr = np.stack([
        rel_value,
        row_rel_abs,
        col_rel_abs,
        (row == col).astype(np.float64),
    ], axis=1).astype(np.float32)
...
def matrix_to_graph(A, name='unknown', topology_hop=1,
    ...
    if feature_mode == 'algebraic':
        node_attr, edge_attr = algebraic_graph_features(A, row, col, values)
...
def main():
    ...
    parser.add_argument('--feature-mode', choices=['legacy', 'algebraic'],
                        default='legacy',
                        help='Graph feature construction mode')
    ...
    for tarpath in non_sym_tars:
        ...
        try:
            graph_data = matrix_to_graph(
                A, name=name, feature_mode=args.feature_mode)
        except Exception as e:
        ...
        for s in range(1, args.samples_per_matrix):
            sample = matrix_to_graph(
                A, name=f'{name}_s{s}', feature_mode=args.feature_mode)
